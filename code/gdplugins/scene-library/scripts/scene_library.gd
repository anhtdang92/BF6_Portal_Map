...
func _enter_tree() -> void:
	...
	popup.add_item("Generate Library", LibraryMenu.GENERATE)
	popup.add_separator()
...
static func create_asset_no_thumbnail(id: int, uid: String, path: String) -> Dictionary[StringName, Variant]:
	return {
		&"id": id,
		&"uid": uid,
		&"path": path,
...
func _create_asset(id: int, uid: String, path: String, skip_thumbnail: bool = false) -> Dictionary[StringName, Variant]:
	return {
		&"id": id,
		&"uid": uid,
		&"path": path,
		&"thumb": _get_or_create_thumbnail(id, path) if not skip_thumbnail else "",
	}
...
func update_item_list() -> void:
	...
	for asset: Dictionary in assets:
		...
		var filter_asset = false
		for query in queries:
			if query.to_lower() not in path.get_file().to_lower():
				filter_asset = true
		if filter_asset:
			continue
